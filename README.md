...
現在はバックエンド API を実装済みで、React / TypeScript frontend は設計済み・未実装です。
...
| Build | Gradle |
| CI | GitHub Actions |
| Frontend | React 19.2.7 / TypeScript 5.9.3 / MUI 9.2.0 / Vite 8.1.4（foundation 実装済み） |
...
- React frontend foundation
  - React / TypeScript / Vite / MUI と pnpm lockfile
  - React Router、TanStack Query、React Hook Form、Zod の依存関係
  - Vitest、Testing Library、MSW の test 基盤
  - ESLint、typecheck、foundation test、production build が成功
- CI / production container
  - pull request と `main` push で GitHub Actions CI を実行
  - 全テスト、executable JAR、テストレポートを自動生成
...
- React フロントエンド
  - Phase 14B で authentication、共通 layout、業務画面、frontend test を実装予定
...
現時点では MUI foundation のみ実装済みで、login と業務画面は Phase 14B の対象です。
...
Phase 12 GitHub Actions / 本番 Dockerfile（完了）
Phase 13 AWS 構成設計（完了）
Phase 14A MUI UI設計 / Review API / frontend foundation（完了）
Phase 14B React 共通基盤 / 業務画面 / frontend test（次フェーズ）
```
