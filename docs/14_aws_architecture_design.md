...
本書は Phase 13 の設計成果物である。Phase 16E で application の S3 adapter まで実装したが、AWS resource の作成、Infrastructure as Code、実環境への deploy は未実施である。
...
| Receipt storage | Amazon S3 private bucket を利用する。DB には非公開 key と metadata/state を保持し、browser へ key を公開しない。Phase 16E で S3 adapter と offline contract test まで実装済み。 |
...
Phase 16B で `receipt_files` V5、storage/scanner port、local adapter、Phase 16C で Receipt Service、Phase 16D で HTTP API、Phase 16E で AWS SDK v2 S3 adapter を実装した。Frontend file UI は未実装であり、現行 UI はまだ `receipt_object_key` を扱う。
...
- Phase 16 は backend application proxy で upload/download を streaming し、既存 session/authorization/audit を毎回適用する。Presigned URL は size/traffic が再検討条件を満たした場合だけ新 ADR で判断する。
- JPEG/PNG/PDF、10 MiB、signature/Content-Type/extension、SHA-256、malware scan、state/cleanup を server で強制する。
- Initial encryption は SSE-S3 とする。Customer-managed KMS key は compliance requirement がある場合に再評価する。
- Phase 16E adapter は exact Content-Length/verified Content-Type、SSE-S3、`If-None-Match: *` を SDK request に設定し、SDK 404/error を storage contract へ変換する。Mock client test のみで、AWS network には接続しない。
- Lifecycle rule による削除は会計証憑の保存要件確定後に設定し、未確定のまま自動削除しない。

Local/private S3 adapter、application proxy、state/recovery の判断は [ADR-002](adr/ADR-002-receipt-storage-and-delivery.md)、詳細は [領収書ファイル設計書](17_receipt_file_design.md) を参照する。
...
- Phase 15 で HTTP Basic を廃止し、Spring Session JDBC、secure cookie、CSRF、session revocation、account lock を実装済みである。Session state は ECS task の memory/local disk に保持しない。組織 IdP が必要な場合は OIDC/MFA を後続候補として再評価する。
...
Application は session state を container memory/local storage に保持しない。Phase 15 の authentication session は PostgreSQL へ外部化し、rolling update、複数 task、task replacement に耐えられる構成とする。業務 API 自体は ECS task 固有の state に依存しない。
...
後続の正式な優先順位、scope、完了条件は `docs/09_phase_plan.md` に定義する。本 AWS design に関係する実施順序は以下とする。

| Order | Phase / Task | Completion criteria |
|---|---|---|
| 1 | Phase 15 / Production authentication（完了） | HTTP Basic を廃止し、container local state に依存しない secure cookie session、CSRF、revocation、account policy を実装・試験済み。 |
| 2 | Phase 16 / 領収書 file API | Private storage adapter、認可済み upload/download、file validation、S3 access design を実装・試験する。 |
| 3 | Phase 17 / Terraform IaC | VPC、ALB、ECS、RDS、ECR、S3、IAM、monitoring が code review と static validation 可能になる。AWS resource はまだ作成しない。 |
| 4 | Phase 18 entry gate | Domain、traffic、RTO/RPO、retention、予算、通知先、AWS account/Region と resource 作成承認が確定する。 |
| 5 | Phase 18 / Deploy | JDBC SSL、secret injection、production log、graceful shutdown、immutable image の ECR push と ECS deployment を検証する。 |
| 6 | Phase 18 / Security / failure test | Access control、secret rotation、task replacement、Multi-AZ failover、DB restore を確認する。 |
| 7 | Phase 18 / Runbook / release | Deploy、rollback、incident、backup restore 手順を作成し、production readiness review を通過する。Remote CI/CD は別途明示承認された場合だけ実装する。 |
